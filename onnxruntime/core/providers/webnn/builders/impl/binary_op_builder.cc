...
bool BinaryOpBuilder::HasSupportedInputsImpl(const GraphViewer&, const Node& node,
  ...
  return IsInputRankSupportedByWebNNOp(node, wnn_limits, logger) && IsDataTypeSupportedByOp(op_type, input0_type, wnn_limits, webnn_input_name, onnx_input_name, logger);
}
